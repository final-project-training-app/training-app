...
import { fallbackExerciseAudioUrl } from "../session/api";
...
export default function HomePage() {
  ...
  async function handleStartCall() {
    ...
    const exerciseAudioUrl =
      cachedSession?.exerciseAudioUrl ?? fallbackExerciseAudioUrl;

    void startSessionAudio(exerciseAudioUrl).catch(() => {
      // SessionPage will make one more attempt if the browser still blocks playback here.
    });

    if (!cachedSession) {
      void queryClient.fetchQuery(queryOptions);
    }
